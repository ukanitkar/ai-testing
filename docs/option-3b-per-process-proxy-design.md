...
| `EXCEL.EXE` | **Directly confirmed**, one tier below Word's precision — a Microsoft Q&A response from an "Independent Advisor" (Microsoft's recognized community-expert role, not an anonymous post, though not an official Microsoft engineering statement either) states: *"Excel uses Microsoft Edge WebView2 Runtime to load web-based features such as Copilot, add-ins, and other cloud experiences."* Scoped to "Copilot" generally, not narrowed to "the pane" the way Word's citation is. |
| `POWERPNT.EXE` | Named directly, but **secondhand-sourced** (via BleepingComputer's reporting on the KB article, see above) — not yet independently confirmed the way Word/Excel now are |
...
- [Why Excel uses a lot of memory even when Copilot is not being used — Microsoft Q&A](https://learn.microsoft.com/en-us/answers/questions/5787547/why-excel-uses-a-lot-of-memory-even-when-copilot-i) — directly read; a Microsoft Q&A "Independent Advisor" response states Excel uses WebView2 Runtime for "Copilot, add-ins, and other cloud experiences." One tier below Word's citation in precision (scoped to Copilot generally, not "the pane" specifically) but independent of the KB article
